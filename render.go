...
char DOC_{{.Name}}[] = "Documentation"; // TODO retrieve documentation
{{end}}
...
static struct PyModuleDef module = {
   PyModuleDef_HEAD_INIT, "my_package", NULL, -1, Methods
};
...
PyInit_api(void)
{
    ...
    generatedTemplate = template.Must(template.New("render").Parse(`// generated by goprotopy -- DO NOT EDIT
package main
...
func getRenderedPath(inputPath string) (string, error) {
    ...
    return filepath.Join(dir, fmt.Sprintf("python/%s.go", file)), nil
}

func getCRenderedPath(inputPath string, moduleName string) (string, error) {
    if !strings.HasSuffix(inputPath, ".go") {
        return "", fmt.Errorf("Input path %s doesn't have .go extension", inputPath)
    }
    trimmed := strings.TrimSuffix(inputPath, ".go")
    dir, _ := filepath.Split(trimmed)
    return filepath.Join(dir, fmt.Sprintf("python/%s.c", inputPath)), nil
}
...
func render(w io.Writer, fullPackageName string, packageName string, functions []GeneratedFunction) error {
    return generatedTemplate.Execute(w, generateTemplateData{fullPackageName, packageName, functions})
}
